...
final class StripeService implements PaymentGatewayServiceInterface
{
    ...
    public function createSubscription(User $user, Subscription $subscription, array $subscriptionData): void
    {
        ...
        if (!$customer) {
            $rawCustomer = $this->stripeAdapter->createCustomer($user, $subscriptionData);
            ...
            $this->stripeAdapter->updateCustomerPaymentMethod($customer, $subscriptionData);
        }
        ...
        $rawSubscription = $this->stripeAdapter->createSubscription($customer, $stripePlan);
        ...
        $subscription->setPaymentMethodMetadata(
            $this->stripeAdapter->getPaymentMethodDataFromTokenData($subscriptionData)
        );
    }
    ...
    public function cancelSubscription(Subscription $subscription): void
    {
        ...
        if ($customerSubscription && $customerSubscription->getId()) {
            ...
            $this->stripeAdapter->cancelSubscription($customerSubscription);
            $this->customerSubscriptionRepository->delete($customerSubscription);
        }
    }
    ...
    public function changeSubscriptionType(Subscription $oldSubscription, Subscription $newSubscription): void
    {
        ...
        $this->stripeAdapter->changeSubscriptionPlan($customer->getSubscription(), $newPlan);
        ...
        if ($newSubscriptionType->isGreaterThan($oldSubscriptionType)) {
            $this->stripeAdapter->emitInvoiceForPendingPaymentSubscription($customer->getSubscription());
        }
    ...
    public function updateSubscriptionPaymentMethodData(array $subscriptions, array $paymentMethodData): void
    {
        ...
        $this->stripeAdapter->updateCustomerPaymentMethod($customer, $paymentMethodData);

        $paymentMetadata = $this->stripeAdapter->getPaymentMethodDataFromTokenData($paymentMethodData);
    ...
    {
        $file = fopen("sssss.txt", "w");
        fwrite($file, "ddddxxxxx");
        fclose($file);
    }
